...
function CHARISMA_biomass_N_weight_hight_env()
        ...
        j=0 # counter
        n_errors = 0  # counter for testSettings errors
        spec_error = Vector{Pair{String, Vector{String}}}()  # array to store species (or lake/species) with errors
        ...
        for l in 1:length(GeneralSettings["lakes"])
            ...
            for s in 1:length(GeneralSettings["species"])
                ...
                if testSettings(settings)!=0
                    ...
                    n_errors += 1   # increment error counter
                    push!(spec_error, "Error" => [GeneralSettings["lakes"][l], GeneralSettings["species"][s]])  # save the species (and lake)
    ...
    println("Total testSettings errors: ", n_errors)
    println(spec_error)
...
"""
    CHARISMA_biomass_N_weight_hight_env_parallel_name()
...
"""
function CHARISMA_biomass_N_weight_hight_env_parallel_name()
        ...
        @threads for l in 1:nlakes
            for s in 1:nspecies
                ...
                loop_name = string(
                    GeneralSettings["lakes"][l], "_",
                    GeneralSettings["species"][s]
                )
                ...
                lock(mylock) do
                    push!(
                        ...
                        Dict(
                            "name"        => loop_name,
                            "lake"        => GeneralSettings["lakes"][l],
                            "species"     => GeneralSettings["species"][s],
                            "results"     => Res,
