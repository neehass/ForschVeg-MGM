...
# Visualise Results
# -------------------------------------------------------------------------------------------
# projekt: dep10_300spec_base_Tsteady_final2.0
# !!! Tprofile muss noch geändert werden, da Tsteady >> also kein Profil, sondern durchgäning Tepi
...
output <- "output/dep10_300spec_base_Tsteady_final2.0"

out_ana_path <- "output-analysis/dep10_300spec_base_Tsteady_final2.0"

save_figures <- "output-analysis/dep10_300spec_base_Tsteady_final2.0/figures"
name <- "Tsteady-base"
...
p_macro
ggsave(file.path(save_figures, paste(name,"_biomass_day.png")), p_macro, height = 10, width = 12)
...
p_box <- ggplot(sort_res, aes(x = factor(depth, levels = rev(sort(unique(depth)))), 
  ...
  guides(color = guide_legend(nrow = 1))
p_box
ggsave(file.path(save_figures, paste(name,"_BOX_biomass_day.png")), p_box, height = 10, width = 12)
...
p_Tprofile_DAY_all
ggsave(file.path(save_figures, paste0(name,"_perAproxMonth.png")), p_Tprofile_DAY_all, height = 10, width = 10)
...
p_Tprofile_DAY
ggsave(file.path(save_figures, paste0(name,"_perAproxMonth2.png")), p_Tprofile_DAY, height = 10, width = 10)
...
sort_env_Tprof_DAY <- sort_env %>% # sort_env[sort_env$day %in% unique(sort_res$day), ] 
  ...
  summarise(
    ...
    metaDepth_av = mean(metaDepth_mean, na.rm = TRUE),
    lakeDepth_av = round(mean(lakeDepth_mean, na.rm = TRUE)), 
    T_prof = list(rep(tempEpi_av, abs(lakeDepth_av*2)+1))) %>% 
  ungroup()
# View(sort_env_Tprof_DAY)
length(sort_env_Tprof_DAY$month_bin %>% unique())
...
p_combo <-  p_box + p_Tprofile + plot_layout(guides = "collect") +
  ...
  theme(legend.position = "bottom")
p_combo
ggsave(file.path(save_figures, paste0("BOX-",name,"_biomass_day.png")), p_combo, height = 10, width = 12)

p_combo <-  p_macro + p_Tprofile + plot_layout(guides = "collect") + 
  plot_layout(ncol = 1, heights = c(3, 1)) &
  theme(legend.position = "bottom")
p_combo
ggsave(file.path(save_figures, paste0(name,"_biomass_day.png")), p_combo, height = 10, width = 12)
