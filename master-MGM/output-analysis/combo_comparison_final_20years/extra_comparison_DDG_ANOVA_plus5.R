...
# dep10_300spec_base_Tprofil_20years_plus5
...
dir_Tprofile_plus5 <- "output-analysis/dep10_300spec_base_Tprofil_20years_plus5"

save_comparison <- "output-analysis/comparison_final_20years/DDG_TprofileVSTsteady"
save_comparison <- "output-analysis/comparison_final_20years/DDG_TprofileVsplus5"
...
res_reshape_Tprofile_plus5 <- readRDS(file.path(dir_Tprofile_plus5, "DDG","DDG_reshape_Tprofile_20years_plus5.rds")) 
res_reshape_Tprofile_plus5 <- res_reshape_Tprofile_plus5[, !names(res_reshape_Tprofile_plus5) %in% "depth_NA"]
#res_reshape_Tsteady <- res_reshape_Tprofile_plus5
...
NSPEC$NSPECbase
lakesDDG_Tprofile$type[lakesDDG_Tprofile$type != "mapped"] <- "model_base_Tprofile"
...
NSPEC_TS$NSPECbase
lakesDDG_Tsteady$type[lakesDDG_Tsteady$type != "mapped"] <- "model_5_Tprofile"
unique(lakesDDG_Tsteady$type)
# load(file.path(save_comparison, paste0("lakesDDG_dep10", scenario, ".RData"))
...
lakesDDG_combo_abDiff <- lakesDDG_combo %>% 
  ...
  mutate(
    ...
    diff_mapped_5Tprofile = mapped - model_5_Tprofile,
    diff_base_5 = model_base_Tprofile - model_5_Tprofile 
  ) %>%
  pivot_longer(
    cols = c(diff_mapped_Tprofile, diff_mapped_5Tprofile, diff_base_5),
    names_to = "comparison",
    values_to = "diff",
  )
...
# diff per depth
p_box_NSpecP_depth_abDiff <- ggplot(lakesDDG_combo_abDiff[lakesDDG_combo_abDiff$comparison == "diff_base_5", ],
  ...
  labs(title = "model Tprofile \nbase - +5°", fill = "Spec.Group")
...
p_box_combo_abDiff
ggsave(file.path(save_comparison, "DDG_abDiff_box_modletype.png"), p_box_combo_abDiff, 
       height = 5.5, width = 5, dpi = "print", scale =1.2)
...
TukeyHSD(anova1) 
# Mapped has significantly lower NSpecP than both model types.
# model_base_Tprofile and +5° do not differ significantly from each other.

# 1. is Nspec sign. differnt between Trpofile base & +5° ?? ------------------------------------
# !!! without mapped !!!
